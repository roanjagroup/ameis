...
class IndexManager
{
    ...
    /*public function return_help()
    {
        ...
        if (!empty($home_menu)) {
            $home_menu_content = '<ul class="nav child_menu">';
    ...
    /**
     * Accepts invitation
     * @author Romell Jaramilo
     * @author Roanja <info@roanja.com> Cleaning code
     * @param add icon fontAwesome
     */
    public function return_help()
    {
        $user_selected_language = api_get_interface_language();
        ...
        if (!empty($home_menu)) {
            ...
            $home_menu_content .= api_to_system_encoding($home_menu, api_detect_encoding(strip_tags($home_menu)));
            $home_menu_content .= '</ul>';
            $html .= self::show_right_block_menu(
                get_lang('MenuGeneral'),
                $home_menu_content,
                'help_block',
                'fa fa-info-circle'
    ...
    {
        $content = '<ul class="nav child_menu">';
        /**
   ...
   /**
    ...
    public function return_skills_links()
    {
        ...
        if (api_get_setting('allow_skills_tool') == 'true') {
            $content .= Display::tag(
                'li',
                Display::url(get_lang('MySkills'),api_get_path(WEB_CODE_PATH).'social/my_skills_report.php'
                )
            );
            $allowSkillsManagement = api_get_setting('allow_hr_skills_management') == 'true';
            if (($allowSkillsManagement && api_is_drh()) || api_is_platform_admin()) {
                $content .= Display::tag('li',
                    Display::url(get_lang('ManageSkills'),
                        api_get_path(WEB_CODE_PATH) . 'admin/skills_wheel.php'));
            }
        }
    ...
    */
    ...
    public function show_right_block_menu(
        $title,
        $content,
        $id = '',
        $icon_font = ''
            ...
            $html = null;
            $html .= '<li id="'.$id.'"><a><i class="'.$icon_font.'"></i> '.$title.' <span class="fa fa-chevron-down"></span></a>'.$content.'</li>' . PHP_EOL;		
    ...
    public function return_classes_block()
    {
        ...
        if (api_get_setting('show_groups_to_users') === 'true') {
            ...
            if (!empty($classes)) {
                $classes = Display::tag('ul', $classes, array('class'=>'nav child_menu'));
    ...
    /*public function return_profile_block()
    {
        ...
        $profile_content = '<ul class="nav child_menu">';
    ...
    public function return_profile_block()
    {
        ...
        if (api_get_setting('allow_message_tool') == 'true') {
            ...
            $profile_content .= '<li class="inbox-message-social"><a href="'.api_get_path(WEB_PATH).'main/messages/inbox.php'.$link.'">'.get_lang('Inbox').$cant_msg.' </a></li>';
    ...
    /*public function return_course_block()
    {
        ...
        // My account section
        $my_account_content = '<ul class="nav child_menu">';
